...
| Full raw scrape payload — **incl. commenter real names and profile photo URLs**, see below | `posts.raw`, `post_comments.raw` | Same | Never read back by the app (confirmed by grep) — kept only as an ingestion artifact. Run `npm run audit:raw-payload` to see what's actually in it. Pruned after `RAW_PAYLOAD_RETENTION_DAYS` (default 90) by the `prune-raw-payloads` cron |
...
4. **Strip the unused Scoutline fields at ingest** (raised 2026-08-20 by the first real `audit:raw-payload` run, not built). ~20 of ~33 fields in `scout_snapshots.raw` are stored and never read — listed under "What raw scrape payload actually contains" above. Both normalizers currently keep the whole actor item as `raw: item`, so the cheap version is an allowlist in `apify-scout-normalize.ts` / `apify-scout-normalize-facebook.ts`.

   **Deliberately not done unprompted**, because unlike everything else in this document it is *irreversible for future data*: a field dropped at ingest cannot be re-derived later, and the stated purpose of keeping `raw` at all is to be the artifact you go back to. The gain is also bounded now that the prune exists — these fields already disappear at 90 days, so stripping buys a shorter window rather than preventing indefinite retention. Worth doing on the clearly-identifying unused fields (`profile_url`, the two post IDs); genuinely a judgement call on the derived metrics. Decide before this data grows, not after.

~~5. Decide a real retention policy for structured data~~ — **resolved 2026-07-30**: see "Retention" above. `post_comments.text`/`author_handle` (the actual third-party personal data in the structured-data set) now prunes on `COMMENT_RETENTION_DAYS`; posts' own caption/engagement data and derived `sentiment` rows are deliberately kept indefinitely, for reasons documented in that section.
