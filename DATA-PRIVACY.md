...
`20260820150000_scout_snapshot_raw_nullable` (added 2026-08-20) is hand-written for the same reason and is **pending as of this writing** — it applies on the next deploy, via the same `prisma migrate deploy` step in `npm run build`. Until it does, the new `scout_snapshots.raw` prune in the cron will fail against a `NOT NULL` column, so deploy rather than backporting just the route change.
